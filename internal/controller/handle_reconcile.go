...
func (r *AIChatWorkspaceReconciler) handleReconcile(ctx context.Context, result *ctrl.Result, aichat *appsv1alpha1.AIChatWorkspace) (*ctrl.Result, error) {
	...
	// Ollama is the LLM-as-a-Service component. Default.
	...
	// Postgres is timescale's version. With pgai support. (Optional)
	if aichat.Spec.Postgres {
	...
	// OpenWebUI Pipelines (Optional)
	if aichat.Spec.OpenWebUIPipelines {
	...
	// OpenWebUI ChatGPT-like interface to privately interact with your local models and N8N agents. Default component.
	if aichat.Spec.OpenWebUI {
	...
	// N8N is a low-code platform with over 400 integrations and AI components. (Optional)
	if aichat.Spec.N8N {
		...
		// TODO: add service and ingress
	}

	// Qdrant an open-source, high performance vector store with an comprehensive API (Optional)
	if aichat.Spec.Qdrant {
